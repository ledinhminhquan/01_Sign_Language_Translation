"""One-button autopilot: data -> baseline -> train -> evaluate -> tune -> error-analysis -> quality
-> benchmark -> demo -> monitoring -> report + slides + grade + bundle.

Each stage is isolated in its own try/except and never aborts. The neural recognizer / t5 translator
training is skipped when torch/transformers are unavailable; everything else degrades to the numpy
centroid recognizer + lexicon translator + SeedPoseEngine + the agent. A zipped bundle is written.
"""
...
def _training_available() -> bool:
    return all(find_spec(m) is not None for m in ("torch",))
...
def _demo_agent(cfg: AppConfig) -> Dict:
    ...
    out: List[Dict] = []
    for ex in samples.seed_eval(cfg, n=10):
        res = agent.translate(ex["seq"])
        out.append({"gold": ex["text"], "pred": res["text"], "glosses": res["glosses"][:4],
...
def run_autopilot(cfg: AppConfig, title: str = None, author: str = None,
    ...
    _step(steps, "prepare_data", lambda: __import__(
        "signlang.data.download_dataset", fromlist=["download_all"]).download_all(cfg))
    _step(steps, "train_baseline", lambda: __import__(
        "signlang.training.train_baseline", fromlist=["build_baseline"]).build_baseline(cfg, limit=limit))

    if train and not can_train:
        logger.info("training requested but torch unavailable - skipping the neural core")
    _step(steps, "train_core", lambda: __import__(
        "signlang.training.train_sign2text", fromlist=["train_all"]).train_all(cfg, limit=limit),
        skip=not can_train)

    _step(steps, "evaluate", lambda: __import__(
        "signlang.training.evaluate", fromlist=["evaluate"]).evaluate(cfg, save=True, load_model=can_train))
    _step(steps, "tune", lambda: __import__(
        "signlang.training.tune", fromlist=["tune"]).tune(cfg, save=True, load_model=can_train))
    _step(steps, "error_analysis", lambda: __import__(
        "signlang.analysis.error_analysis", fromlist=["error_analysis"]).error_analysis(cfg, save=True))
    _step(steps, "quality", lambda: __import__(
        "signlang.analysis.quality", fromlist=["quality_report"]).quality_report(cfg, save=True))
    _step(steps, "benchmark", lambda: __import__(
        "signlang.analysis.latency", fromlist=["benchmark"]).benchmark(cfg, n=8, warmup=2, save=True))
    _step(steps, "demo_agent", lambda: _demo_agent(cfg))
    _step(steps, "monitoring", lambda: __import__(
        "signlang.monitoring.drift_report", fromlist=["monitoring_report"]).monitoring_report(cfg))

    stamp = utc_stamp()
    sub = artifacts_dir() / "submission" / f"submission-{stamp}"
    sub.mkdir(parents=True, exist_ok=True)
    report = _step(steps, "report", lambda: __import__(
        "signlang.autoreport.report_pdf", fromlist=["generate_report"]).generate_report(
        cfg, title=title, author=author, out_path=sub / "report.pdf"))
    slides = _step(steps, "slides", lambda: __import__(
        "signlang.autoreport.slides_pptx", fromlist=["generate_slides"]).generate_slides(
        cfg, title=title, author=author, out_path=sub / "slides.pptx"))

    repo_root = Path(__file__).resolve().parents[3]
    checklist = _step(steps, "grading", lambda: __import__(
        "signlang.grading.checklist", fromlist=["build_checklist"]).build_checklist(repo_root))
