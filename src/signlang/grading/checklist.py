...
_SUBPACKAGES = ["data", "pose", "models", "segmentation", "training", "agent", "api",
                "analysis", "autoreport", "monitoring", "automation", "grading"]
_REQUIRED_SRC = [
    f"src/{PKG}/config.py", f"src/{PKG}/cli.py", f"src/{PKG}/logging_utils.py",
    f"src/{PKG}/pose/engine.py", f"src/{PKG}/pose/layout.py",
    f"src/{PKG}/data/synth_pose.py", f"src/{PKG}/data/lexicon.py", f"src/{PKG}/data/samples.py",
    f"src/{PKG}/data/dataset.py", f"src/{PKG}/models/sign2text.py", f"src/{PKG}/models/baseline.py",
    f"src/{PKG}/segmentation/segmenter.py", f"src/{PKG}/training/train_sign2text.py",
    f"src/{PKG}/training/evaluate.py", f"src/{PKG}/training/metrics.py",
    f"src/{PKG}/agent/translate_agent.py", f"src/{PKG}/agent/policy.py", f"src/{PKG}/api/main.py",
]
...
_REQUIRED_DOCS = [
    ...
    "ethics_statement", "architecture", "translation_evaluation", "model_card", "slide_deck_outline", "DESIGN_BRIEF",
]

_REQUIREMENTS = {
    "R1_problem_data": ("Problem definition + dataset/data card",
                        ["docs/problem_definition.md", "docs/data_description.md",
                         "docs/data_card.md", f"src/{PKG}/data/synth_pose.py"]),
    "R2_model_baseline": ("Model selection + baseline vs the trained core",
                          ["docs/model_selection.md", f"src/{PKG}/models/sign2text.py",
                           f"src/{PKG}/models/baseline.py"]),
    "R3_training_eval": ("Training + evaluation with metrics (BLEU/chrF + gloss WER)",
                         [f"src/{PKG}/training/train_sign2text.py", f"src/{PKG}/training/evaluate.py",
                          "docs/translation_evaluation.md"]),
    "R4_agent": ("Agentic pipeline with explicit decision points",
                 [f"src/{PKG}/agent/translate_agent.py", f"src/{PKG}/agent/policy.py", "docs/agent_architecture.md"]),
...
def build_checklist(repo) -> Dict:
    ...
    check("Functional: evaluate() produces metrics", eval_res["ok"],
...
def _check_import() -> Dict:
    try:
        import importlib
        for m in (PKG, f"{PKG}.config", f"{PKG}.agent.translate_agent", f"{PKG}.training.evaluate"):
...
def _check_agent_offline() -> Dict:
    try:
        ...
        ex = samples.seed_eval(cfg, n=1)[0]
        job = agent.run(ex["seq"], save=False)
        data = job.to_dict()
...
def _check_evaluate() -> Dict:
    try:
        ...
        res = evaluate(_offline_cfg(), save=False, load_model=False, limit=20)
        systems = res.get("systems") or {}
        ok = "agent" in systems and "translation" in systems["agent"]
