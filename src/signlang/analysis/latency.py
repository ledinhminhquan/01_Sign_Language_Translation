"""Latency benchmark: end-to-end sign-translation agent throughput (offline, centroid recognizer)."""
...
def benchmark(cfg: AppConfig = None, n: int = 10, warmup: int = 2, save: bool = True) -> Dict:
    ...
    try:
        ...
        ev = samples.seed_eval(cfg, n=max(n + warmup, 8))
    except Exception as exc:
        logger.warning("benchmark: could not build agent (%s)", exc)
        out = {"device": device, "n": n, "error": str(exc), "latency_ms": _pct([]),
               "throughput_per_s": 0.0, "decision_presence": {}, "statuses": {}}
        if save:
            _save(out)
        return out

    seqs = [ex["seq"] for ex in ev]
    for s in seqs[:warmup]:
        try:
            agent.run(s, save=False)
        except Exception:
            pass
    ...
    n_ok = 0
    for s in seqs[warmup:warmup + n]:
        t0 = time.perf_counter()
        try:
            job = agent.run(s, save=False)
            ...
            for d in job.decisions:
                decisions[d.id] = decisions.get(d.id, 0) + 1
        except Exception as exc:
    ...
    out = {"device": device, "n": n_ok, "warmup": warmup,
           "recognizer": getattr(agent.recognizer, "name", "centroid"),
           "latency_ms": _pct(lat),
